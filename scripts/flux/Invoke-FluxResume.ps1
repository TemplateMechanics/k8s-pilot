<#
.SYNOPSIS
    Resume reconciliation on a Flux resource (CLAUDE.md §3.4 / §3.6).

.DESCRIPTION
    Metadata-only mutation per CLAUDE.md §3.6 (exempt from the diff-artifact
    rule because the change IS the parameter — flips `spec.suspend: false`).
    Still requires explicit `-Context` and a meaningful `-Reason` recorded
    in the local audit log.

    After resume, the first reconcile may apply accumulated drift. Run
    Invoke-FluxDiff.ps1 first if the suspension was long.

.PARAMETER Kind
    Flux CR kind. Same allowed set as Invoke-FluxSuspend.ps1.

.PARAMETER Name
    Flux CR name.

.PARAMETER Reason
    Required free-text reason (>=5 chars).
...
.OUTPUTS
    flux's stdout. Exit codes:
      ...
      other - propagated from `flux resume`
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
$logFile = Join-Path $auditDir 'resume.log'
...
Write-Information "Resuming $Kind/$Name on context '$Context' (reason: $Reason). The first reconcile may apply accumulated drift." -InformationAction Continue
& flux resume $Kind $Name --context $Context
$exit = $LASTEXITCODE
...
if ($exit -ne 0) {
    Write-Error "flux resume $Kind $Name failed (exit $exit). Audit log: $logFile"
    exit $exit
}
Write-Information "Resume complete. Audit log: $logFile" -InformationAction Continue
exit 0
