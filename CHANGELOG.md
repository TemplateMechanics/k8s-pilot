...
- `scripts/argocd/Invoke-ArgocdLogin.ps1`: `argocd login <server>` with optional SSO/username+password/insecure. Mutually-exclusive SSO/credentials check. CLI session stored in the user's standard config dir (cross-platform repo-local relocation is out of scope for this PR; the diff/sync wrappers use `.argocd/` only for artifacts).
- `scripts/argocd/Invoke-ArgocdAppDiff.ps1`: `argocd app diff --revision --server --exit-code`; emits `.argocd/<serverSlug>/<appSlug>/<revision>.diff` + `.diff.meta.json` sidecar (schemaVersion=1; server/app/revision/diffExitCode/generatedAt). Captures stderr separately so warnings cannot pollute the diff artifact. Translates argocd's exit 1 ("diff present") to wrapper exit 2 so callers branch consistently with helm-diff/kubectl-diff.
